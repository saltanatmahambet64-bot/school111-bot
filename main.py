...
TEXTS = {
    "ru": {
        ...
        "send_photo_error": "⚠️ Пожалуйста, отправьте фотографию вещи (картинкой) или нажмите кнопку «Отмена» / «Пропустить фото».",
        ...
        "info": (
            ...
            "🤝 Давайте вместе сделаем нашу школу честнее и добрей!"
        ),
    ...
    "kk": {
        ...
        "send_photo_error": "⚠️ Өтінемін, заттың фотосуретін жіберіңіз немесе «Болдырмау» / «Фотоны өткізу» түймесін басыңыз.",
...
def get_db_connection():
    # Render передает строку подключения в переменной DATABASE_URL
...
def add_pending_item(user_name: str, description: str, photo_id: str) -> int:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO found_items (user_name, description, photo_id, status) VALUES (%s, %s, %s, 'pending') RETURNING id",
...
def approve_item(item_id: int):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE found_items SET status = 'approved' WHERE id = %s", (item_id,))
    conn.commit()
    cursor.close()
    conn.close()
...
@dp.message(FoundItemState.waiting_for_photo)
async def process_found_photo_error(message: types.Message):
    lang = user_languages.get(message.from_user.id, "ru")
    await message.answer(TEXTS[lang]["send_photo_error"])
...
@dp.message(LostItemState.waiting_for_photo)
async def process_lost_photo_error(message: types.Message):
    lang = user_languages.get(message.from_user.id, "ru")
    await message.answer(TEXTS[lang]["send_photo_error"])
...
async def main():
    print("Бот Бюро Находок Школы №111 переведен на PostgreSQL и успешно запущен!")
